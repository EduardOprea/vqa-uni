...
class VQAWithAnswerInfoModel(nn.Module):
    def __init__(self, visual_bert: VisualBertModel, cls, config):
        super().__init__()
        ...
        # VQA additional answer head
        self.answer_known_classifier = nn.Linear(visual_bert_hidden_dim  + 2, 2)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(
        ...
        confidence_score: Optional[torch.FloatTensor] = None
    ) -> Tuple[torch.Tensor]:
        ...
        logits = self.cls(pooled_output)
        reshaped_logits = logits.view(-1, self.config.num_labels)
        entropy = entropy.view(-1,1,1)
        confidence_score = entropy.view(-1,1,1)
        input_answer_known_cls = torch.cat((pooled_output, entropy, confidence_score), 2)
        logits_know_answer = self.answer_known_classifier(input_answer_known_cls)
        reshaped_logits_know_answer = logits_know_answer.view(-1, 2)
...
class VQAJustVisualFeaturesModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.visual_embeds_dim = 36*2048
        self.dropout = nn.Dropout(0.1)
        # VQA additional answer head
        self.flatten = nn.Flatten()
        self.answer_known_classifier =  nn.Linear(self.visual_embeds_dim, 2)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(
        ...
        flattened_visual_embeds = self.flatten(visual_embeds)
        flattened_visual_embeds = self.dropout(flattened_visual_embeds)
        logits_know_answer = self.answer_known_classifier(flattened_visual_embeds)
        reshaped_logits_know_answer = logits_know_answer.view(-1, 2)
        
        reshaped_logits_know_answer = self.sigmoid(reshaped_logits_know_answer)

        return reshaped_logits_know_answer
...
def compute_target_know_answer(answers_dset, ans2label, name, cache_root='../data/cache'):
    """Create labels for questions if the the answers is known (among the most common answer) or unknown
    ...
    ***answers_dset should be preprocessed***
    ...
    for ans_entry in answers_dset:
        ...
        most_given_answer = max(answer_count, key=answer_count.get)
        if most_given_answer not in ans2label:
            # if the answer is not amongst the most usual ones, label it as unknown answer
            label = 0
        else:
            label = 1
